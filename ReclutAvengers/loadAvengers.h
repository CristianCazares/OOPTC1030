#ifndef loadAvengers_h
#define loadAvengers_h
...
void loadAvengers(vector<string> arr){
    ...
    for(int i=0;i<arr.size();i++){
        ...
        if(data[0]=="H"){
            ...
            Avengers.push_back(new Human(name, alias, gender, country, colors, powers));

            Human aux(name, alias, gender,country, colors, powers);
            Humans.push_back(aux);
        }else if(data[0]=="A"){
            string planet = data[4];
            Avengers.push_back(new Alien(name, alias, gender, planet, colors, powers));
